...
With an understanding of the TPG, ORA, and calculation circuitry of the BIST, the last import aspect to be famillar with is how the BIST is controlled and observed (recall that Section \ref{sct:whytest} statess that controllability and observability directly influence the testability of a circuit).  Excluding some run flags, the majority of the BIST is controlled via a SPI interface.  A quick overview of this SPI interface is important to understand the BIST model and it will be explained at a high-level.  The input to the BIST consists of an 88-bit SPI write which includes four 15-bit frequency words $FW_1$ through $FW_4$, the number of samples, and a test control word (TCW)\nomenclature{TCW}{Test Control Word} specifying which test to run.  $FW_1$ and $FW_2$ always control the two frequencies generated by the BIST DDS circuitry.  $FW_3$ and $FW_4$ have different meanings depending on the test run and will be explained later as specific tests are discussed.  The number of samples is optional and will only be used if a spur search, SNR, or NF test is being executed as discussed in Section \ref{sct:onchip}.  The TCW includes the test to be run as well as some additional flags including those which optionally selecting the use of the digital loopback or bypass path and a bit to enable the use of the half-imp accumulation time.  
...
In addition to the SPI IO, there are a few other important outputs used in simulation.  First there is the DDS output which is the generated tone for the test being executed, there is also a 10-bit test result output which directly outputs the ``Log Result'' word from the SPI once a test has been completed.  Finally, there is a ``Done'' flag used to denote that the BIST has finished performing the test requested and that the results are ready for retrieval.
