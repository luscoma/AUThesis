...
\begin{table}
  ...
  \end{center}
\end{table}
The last two columns correspond to the number of clock cycles each test requires.  The \textit{Accumulation Cycles} column is the number of clock cycles that a single measurement will require.  This is directly related to the values of the frequency words used and is explained in Section \ref{sct:bist-ora}.  The final column is the total number of clock cycles required to perform the fault simulation.  This incorporates the number of samples taken (if applicable) and clock cycles used to write to and read from the BIST SPI interface.

Fault simulations were performed for each of the measurements shown in Table \ref{tbl:faulttests}.  For each separate simulation the entire fault list containing 39501 faults was used to determine the individual fault coverage of each test.  Using the entire fault list for each test causes a large increase in fault simulation time (recall that Equation \ref{equ:fsimtime} shows that fault simulation time increases as the number of faults to simulate increases); however, due to available computing power this was the preferred approach and will be discussed in more detail in the next section.
