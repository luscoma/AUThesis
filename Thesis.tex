...
Once the netlist is converted to ASL using the VerilogParser tool, test vectors must be developed to perform a given measurement.  In the case of the SSA BIST these vectors can be complex and require verification to ensure that they are valid and produce the expected circuit output.  Unfortunately, verification of these test vectors can be difficult to perform manually via visual inspection.  This is especially true for circuits which require a large number of test vecotrs and/or have a large number of inputs and ouputs.  The AUSIM out file format is straight forward and is formatted such that each line of text represents a test vector which was applied and the output of the circuit after the application of the test vector.  Consequently, the output file consists of all binary patterns which are difficult to distinguish by inspection.  Instead of manual verification, it would be beneficial to convert this format to a standard format for viewing in a graphical environment.  To do this a tool OutToVCD was written which converts the standard out file format to a Value Change Dump file (VCD) file, this conversion makes verification via inspection much simpler and can greatly reduce the time required for test vector verification significantly.
...
\end{table}
This will convert the logic simulation output of c17 and to a VCD file which can be viewed in GTKWave.  The `-g' switch provides a list of outputs which should be concatenated and treated as a bit vector instead of individual bits.  This switch allows for easier verification of multi-bit values.
