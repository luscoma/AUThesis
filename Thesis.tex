...
Once the netlist is converted to ASL using the VerilogParser tool, test vectors must be developed to perform a given measurement.  For the SSA BIST these vectors include writing the proper SPI words to run the appropriate measurement and subsequently waiting for the require number of clock cycles before reading the results out of the SPI.  These vectors can be fairly complex and require verification to ensure that they produce the expected circuit output and wait the appropriate number of clock cycles.  Unfortunately, verification of these test vectors can be difficult to perform manually via visual inspection.  This is especially true for circuits which require a large number of test vectors and/or have a large number of inputs and ouputs.  The AUSIM out file format is straight forward and is formatted such that each line of text represents a test vector which was applied and the output of the circuit after the application of the test vector.  Consequently, the output file consists of all binary patterns which are difficult to distinguish by inspection.  Instead of verification of the text based output, it would be beneficial to convert this format to a standard format for viewing in a graphical environment.  To do this a tool OutToVCD was written which converts the standard out file format to a Value Change Dump file (VCD) file, this conversion makes verification much simpler and significantly reduces the time required for test vector verification.
